...
void ARoomActor::DrawPathDebug(const FRoomInteriorLayout& Layout) const
{
	UWorld* World = GetWorld();
	if (!World)
	{
	...
	for (const FRoomInteriorPath& Path : Layout.GuaranteedPaths)
	{
		...
		{
			const FVector A = GetActorTransform().TransformPosition(Path.PathPoints[i]);
			const FVector B = GetActorTransform().TransformPosition(Path.PathPoints[i + 1]);
			DrawDebugLine(World, A + FVector(0,0,100), B + FVector(0,0,100), FColor::Green, false, 15.f, 0, 6.f);
		}

		if (Path.bClosedLoop && Path.PathPoints.Num() > 1)
		{
			const FVector A = GetActorTransform().TransformPosition(Path.PathPoints.Last());
			const FVector B = GetActorTransform().TransformPosition(Path.PathPoints[0]);
			DrawDebugLine(World, A + FVector(0,0,100), B + FVector(0,0,100), FColor::Green, false, 15.f, 0, 6.f);
		}
	}
}
